...
static F32 dB_to_linear(F32 dB)
{
  ...
  return (F32) pow( 10.0, ( dB / ( 20000.F / 6.0f ) ));
}
...
static F32        AILEXPORT M3D_3D_sample_volume(H3DSAMPLE samp)
{
   ...
   return( (F32)pow( S->volume, 6.0 / 10.0 ) );
}
...
#ifdef EAX3
         AIL_set_error("Unable to initialize EAX 3 support");
#elif defined EAX2
...
#else
extern "C" S32 mss3dMain(HPROVIDER provider_handle, U32 up_down);
...
#endif
{
   const RIB_INTERFACE_ENTRY M3DSAMPLE[] =
      ...
      REG_AT("EAX sample all parameters",                EAX_EFFECT_ALL_PARAMETERS,               RIB_FLOAT),

      REG_PR("EAX sample reverb mix",                    EAX_EFFECT_REVERBMIX,                    RIB_FLOAT),
      REG_PR("EAX sample all parameters",                EAX_EFFECT_ALL_PARAMETERS,               RIB_FLOAT),
   ...
   const RIB_INTERFACE_ENTRY M3D[] =
      ...
      REG_AT("EAX all parameters",          EAX_ALL_PARAMETERS,                 RIB_FLOAT),
      ...
      REG_PR("EAX all parameters",          EAX_ALL_PARAMETERS,                 RIB_FLOAT),
...
#if defined(EAX3) || defined(DX7SN) || (defined(SWARE) && !defined(DX7)) || defined(AUREAL)
...
#ifndef A1_NO_STATIC
          if (!MSSDllMain(hinstDll, DLL_PROCESS_ATTACH, nullptr))
          {
              return FALSE;
          }
#endif
