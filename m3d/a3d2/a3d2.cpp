...
struct SAMPLE3D
{
   ...
   F32         exclusion;              // amount to attenuate reflections too
...
static LPA3D4        lpA3D;
static LPA3DLISTENER lpA3DListener;
static LPA3DGEOM     lpA3DGeom;
static LPA3DMATERIAL lpA3DWallMaterial;

#define     X_VAL(A3DVEC)     (A3DVEC)[0]
#define     Y_VAL(A3DVEC)     (A3DVEC)[1]
#define     Z_VAL(A3DVEC)     (A3DVEC)[2]
static A3DVECTOR   listen_position;
static A3DVECTOR   listen_face;
static A3DVECTOR   listen_up;
static A3DVECTOR   listen_velocity;
static A3DVECTOR   room_vertices[8];
...
static A3DVECTOR open_air = { 0.0F, 1.0F, 1.0F, 1.0F };
static A3DVECTOR carpet   = { 0.4F, 0.4F, 0.4F, 0.4F };
static A3DVECTOR water    = { 0.7F, 0.3F, 0.8F, 0.3F };
static A3DVECTOR wood     = { 0.8F, 0.7F, 0.5F, 0.8F };
static A3DVECTOR brick    = { 0.9F, 0.8F, 0.3F, 0.9F };
static A3DVECTOR glass    = { 0.9F, 0.9F, 0.2F, 1.0F };
static A3DVECTOR metal    = { 1.0F, 0.9F, 0.6F, 0.9F };

static S32         speaker_type;

static S32         listen_auto_update        = 0;
static S32         flush_timeout             = 30;
static S32         g_room_type;                // values defined by EAX.H
static F32         room_size;                // <= 0 means no virtual geometry
static F32         units_per_meter           = 1.0F;
...
static HANDLE m_lpThread=0;                                         // Thread for buffer maintenance
static HANDLE m_hmaster=0;                                          // master win32 event
static HANDLE m_hBufferEvents[NUM_EVENTS*N_SAMPLES+1]={0};          // Buffer events for this sample
static volatile S32 is_safe;                                        // Thread is waiting for next event
static volatile SAMPLE3D FAR* current_sample;
...
static void AILEXPORT M3D_set_3D_room_type (S32 room_type);
...
static void reset_sample_events(SAMPLE3D FAR *S)
{
   S32 j;
...
static void flush_sample(SAMPLE3D FAR *S)
{
...
static void destroy_all_sample_events(void)
{
   if (m_lpThread)
   {
...
static DWORD WINAPI pfnThreadProc(LPVOID pParam)
{
...
static DWORD WINAPI flushThreadProc (LPVOID pParam)
{
   DWORD dwReturnedEvent = 0;
...
static M3DRESULT AILEXPORT M3D_activate                 (S32 enable)
{
   ...
   if (enable)
      ...
      if (lpA3D->IsFeatureAvailable (A3D_1ST_REFLECTIONS))
         ...
         result = lpA3DWallMaterial->SetNameID ((LPSTR)"walls");
         CHECK_RESULT(result);
         }
