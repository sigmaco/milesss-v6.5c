...
static ROOM_PARAMS rooms[EAX_ENVIRONMENT_COUNT]  =
{
  ...
  { /*EAX_ENVIRONMENT_PADDEDCELL,*/ 0.25F,0.1F,0.0F, 0.002f  },
  { /*EAX_ENVIRONMENT_ROOM,*/ 0.417F,0.4F,0.666F, 0.003f },
  { /*EAX_ENVIRONMENT_BATHROOM,*/ 0.75F,1.499F,0.166F, 0.011f },
  { /*EAX_ENVIRONMENT_LIVINGROOM,*/ 0.208F,0.478F,0.0F, 0.004f },
  ...
  { /*EAX_ENVIRONMENT_PSYCHOTIC,*/ 0.486F,7.563F,4.0F, 0.011f }
};
                      ...
                      S32     out_pos)
{
  ...
  if (S->pipeline[DP_ASI_DECODER].active)
  {
    ...
    if ( needed_src == 0 )
    {
      ...
      if ( S->src_fract + playback_ratio < 65536 )
      {
       handle_zero_compressed:
        //
        // since we need the next sample to bilinear to, when we aren't
        //  going to even eat one src sample, then just use the old sample
        //  to avoid having to decompress the next sample over and over
        //  and over again...
        //

        STORE_LE_SWAP16( &( ( S16 FAR* ) process_buffer)[ 0 ], ((S16)S->left_val) );
        STORE_LE_SWAP16( &( ( S16 FAR* ) process_buffer)[ 1 ], ((S16)S->right_val) );
        goto no_decomp;
      }
  ...
  else
  {
    ...
    if (S->format & DIG_F_ADPCM_MASK)
    {
      if ( needed_src == 0 )
      {
        ...
        //
        // if we aren't going to wrap into the next compressed sample,
        // don't decode it - just use the last sample we did get
        //
...
void AILCALL SS_stream_to_buffer(HSAMPLE S)
{
   S32 out_pos;
   S32 in_len;
   S32 next_buffer;
