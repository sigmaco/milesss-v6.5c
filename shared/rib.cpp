...
RIBDEF HPROVIDER RIBEXPORT RIB_load_provider_library (C8 const FAR *filename)
{
   ...
   S32 (*initfunc)( HPROVIDER provider_handle, U32 up_down );
   initfunc = (S32 (*)(HPROVIDER, U32))dlsym(module, "MSS_RIB_Main");
   if (initfunc == NULL)
      {
      dlclose(module);
      RIB_free_provider_handle(provider);
   ...
   initfunc(RIB_provider_library_handle(), 1);
...
RIBDEF void RIBEXPORT RIB_free_provider_library (HPROVIDER provider)
{
   ...
   if (provider == (HPROVIDER) NULL)
      ...
      if (provider_list.array[provider].module != 0)
         ...
         #else
         #ifdef IS_LINUX
           void *module = (void *)provider_list.array[provider].module;
           S32 (*finifunc)( HPROVIDER provider_handle, U32 up_down );
           finifunc = (S32 (*)(HPROVIDER, U32))dlsym(module, "MSS_RIB_Main");
           finifunc(RIB_provider_library_handle(), 0);
           dlclose(module);
         #endif
         #endif
         #endif
