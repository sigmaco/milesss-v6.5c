                                ...
                                AILLENGTHYCB callback)
{
   ...
   RIB_INTERFACE_ENTRY ASISTR[] =
      ...
      RIB_AT("Input bit rate",           INPUT_BIT_RATE),
      RIB_AT("Input sample rate",        INPUT_SAMPLE_RATE),
      RIB_AT("Input sample width",       INPUT_BITS),
      RIB_AT("Input channels",           INPUT_CHANNELS),
      RIB_AT("Output bit rate",          OUTPUT_BIT_RATE),
      RIB_AT("Output sample rate",       OUTPUT_SAMPLE_RATE),
      RIB_AT("Output sample width",      OUTPUT_BITS),
      RIB_AT("Output channels",          OUTPUT_CHANNELS),
      RIB_AT("Position",                 POSITION),
      RIB_AT("Percent done",             PERCENT_DONE),
      RIB_AT("Minimum input block size", MIN_INPUT_BLOCK_SIZE),
                               ...
                               AILLENGTHYCB callback)
{
   ...
   while (1)
      ...
      if (!comp_ASI || ((info.data_len/LE_SWAP16( &PCM->alignment ) ) < (U32) AIL_get_preference(DLS_ADPCM_TO_ASI_THRESHOLD)))
         {
