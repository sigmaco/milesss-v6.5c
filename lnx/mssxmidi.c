...
// Some defines to get the code up and running
#define WINAPI
#define BYTE unsigned char
#define midiOutOpen(a, b, c, d, e)			-1
#define midiOutReset(hMidiOut)
#define midiOutClose(hMidiOut)
#define midiOutShortMsg(hMidiOut, dwMsg)
#define midiOutLongMsg(hMidiOut, dwMsg)
#define midiOutPrepareHeader(hMidiOut, header, size)	-1
#define midiOutUnprepareHeader(hMidiOut, header, size)
...
S32 AILCALL AIL_API_init_sequence(HSEQUENCE S, void const FAR *start, S32 sequence_num)
{
   ...
   i = 8 + BE_SWAP32( ((U8 FAR *) ((BYTE FAR *) S->TIMB+4)));
   if ( i > sizeof(TIMB) ) {
      i = sizeof(TIMB);
   }
